...
string cosntruir_sub(int index, int tam) {
    string res;
    for (int i=1; i<=tam; i++) {
        res += s[index];
...
int main(){
    ...
    //el arreglo indices contiene los sufijos ordenados, por lo tanto, debemos buscar ahi
    int t;
    cin>>t;
    ...
    while (t--) {
        ...
        while (ini <= fin) {
            ...
            string sub_mit = cosntruir_sub(indices[mit], aux.size());
            if (sub_mit == aux) {
                puede = true;
                break;
            }

            if (aux > sub_mit) ini = mit + 1;
            else fin = mit - 1;
        }
